...
def parse(file):
    ...
    if not loc.is_file():
        print(
            f"{Colors.BOLD_RED}error: {Colors.RESET}{Colors.HI_RED}file {loc} doesn't exist{Colors.RESET}"
        )
        if loc.is_dir():
            print(
                f"{Colors.BOLD_YELLOW}note: {Colors.RESET}{Colors.HI_YELLOW}{loc} is a directory - did you mean a file in it?{Colors.RESET}"
            )
        sys.exit(1)
